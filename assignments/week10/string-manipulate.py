...
x = text.count('a') + text.count('A') + text.count('e') + text.count('E')
+ text.count('i') + text.count('I') + text.count('o') + text.count('o') + text.count('u') + text.count('U')

y = ""
for char in text:
    if char in ['a','e','i','o','u','A','E','I','O','U']:
        y +=char
print("Vowels : %d(%s)"%((x),y[0:-2]))
print(f"--Consonants:{len(text)- text.count('  ')-x}")